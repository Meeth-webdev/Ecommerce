...
function Page() {
  ...
  const getProducts = async () => {
    try {
      const response = await axios.post("/api/price-filter/lowest-highest");
  ...
  <div className="flex items-center gap-2">
     <Repeat className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12" />
        <Link href="/filter-highest-lowest" className="text-xl sm:text-2xl md:text-3xl font-bold hover:underline">
         Highest-Lowest
          </Link>
        </div>
          </div>
